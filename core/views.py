...
def home(request):
    ...
    featured_currency_packs = Product.objects.filter(
        product_type="currency", is_active=True, featured=True
    ).order_by("price")[
        :6
    ]  # Order by price, limit to 6
...
def admin_sales_dashboard(request):
    ...
    recent_orders = Order.objects.filter(
        payment_status="paid", order_date__gte=thirty_days_ago
    ).count()
    ...
    latest_orders = Order.objects.filter(payment_status="paid").order_by(
        "-order_date"
    )[:10]
    ...
    context = {
        ...
        "average_order_value": (
            round(average_order_value, 2) if average_order_value else 0
        ),
