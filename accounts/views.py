...
def order_history(request):
    ...
    orders = (
        ...
        .prefetch_related(
            "items__product", "items__variant", "items__license_keys__variant"
        )
        .order_by("-order_date")
    )
...
def order_detail(request, order_number):
    ...
    order = get_object_or_404(
        Order.objects.prefetch_related(
            "items__product", "items__variant", "items__license_keys"
        ),
    ...
    context = {
        "order": order,
        "items_with_keys": [
            {"item": item, "license_keys": item.license_keys.all()}
...
def wishlist(request):
    ...
    items = Wishlist.objects.select_related(
        "product", "product__currency", "variant"
...
def toggle_wishlist(request, product_id):
    ...
    if variant_id:
        try:
            ...
            return JsonResponse(
                {"success": False, "message": "Invalid variant specified"}
            )
    ...
    return JsonResponse(
        {"success": True, "in_wishlist": in_wishlist, "message": message}
    )
