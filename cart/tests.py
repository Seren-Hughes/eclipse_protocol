...
class CartModelTests(TestCase):
    ...
    def setUp(self):
        ...
        CurrencyProduct.objects.get_or_create(
            product=self.currency_product,
            defaults={'credit_amount': 1000}
        )
    ...
    def test_cart_total_items_with_items(self):
        ...
        cart = Cart.objects.create(user=self.user)
        CartItem.objects.create(cart=cart, product=self.base_game, variant=self.variant, quantity=2)
        CartItem.objects.create(cart=cart, product=self.currency_product, quantity=3)
    ...
    def test_cart_total_price_with_items(self):
        ...
        cart = Cart.objects.create(user=self.user)
        CartItem.objects.create(cart=cart, product=self.base_game, variant=self.variant, quantity=1)
        CartItem.objects.create(cart=cart, product=self.currency_product, quantity=2)
    ...
    def test_cart_item_string_representation(self):
        ...
        self.assertEqual(str(item_without_variant), f"{self.currency_product.name} x3")
...
class CartContextProcessorTests(TestCase):
    ...
    def test_cart_with_variant_product(self):
        ...
        self.assertEqual(context['total'], Decimal('59.99'))  # Uses variant price
...
class CartFunctionalTests(TestCase):
    ...
    def test_complete_shopping_workflow_authenticated(self):
        ...
        response = self.client.get(reverse('cart:cart'))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context['total_items'], 3)  # 1 game + 2 currency
    ...
    def test_complete_shopping_workflow_anonymous(self):
        ...
        response = self.client.get(reverse('cart:cart'))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context['total_items'], 4)  # 1 game + 3 currency
    ...
    def test_multiple_variants_same_product(self):
        ...
        expected_total = Decimal('59.99') + Decimal('49.99')  # PC Ultimate + Xbox Standard
        self.assertEqual(cart.total_price, expected_total)