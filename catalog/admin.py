...
class DigitalVariantInline(admin.TabularInline):
    ...
    extra = 1
    fields = ('platform', 'edition', 'image', 'description', 'sku', 'price_override', 'requires_key', 'is_active', 'sort_order')
...
class ProductAdmin(admin.ModelAdmin):
    ...
    list_display = ('name', 'sku', 'product_type', 'image_preview', 'display_price', 'variant_count', 'is_active', 'featured')
    list_filter = ('product_type', 'is_active', 'featured')
    search_fields = ('name', 'sku')
    prepopulated_fields = {'slug': ('name',)}  # Auto-generate URL-friendly slug
    ordering = ('product_type', 'name')
    
    def get_queryset(self, request):
        """Add variant count annotation for efficient list display"""
        return super().get_queryset(request).annotate(_variant_count=Count('digital_variants'))
    ...
    def _validate_base_game_product(self, request, obj):
        ...
        if variant_count == 0:
            ...
            active_variants = obj.digital_variants.filter(is_active=True).count()
    ...
    def get_inlines(self, request, obj=None):
        ...
        inline_map = {
            Product.BASE_GAME: [DigitalVariantInline],  # Platform/edition variants
            Product.DIGITAL: [DigitalProductInline],    # Simple digital product config
...
class DigitalVariantAdmin(admin.ModelAdmin):
    ...
    """
    list_display = ('product_name', 'platform', 'edition', 'image_preview', 'sku', 'effective_price', 'is_active', 'sort_order')
    ...
    fieldsets = (
        ...
        ('Variant Configuration', {
            'fields': ('platform', 'edition', 'image', 'description', 'sku', 'requires_key')
        }),
    ...
    def image_preview(self, obj):
        ...
        if image:
            from django.utils.html import format_html
            return format_html('<img src="{}" style="max-height:50px; max-width:50px;">', image.url)
        return "No image"
...
class CurrencyProductAdmin(admin.ModelAdmin):
    ...
    def image_preview(self, obj):
        ...
        if obj.product.image:
            from django.utils.html import format_html
            return format_html('<img src="{}" style="max-height:50px; max-width:50px;">', obj.product.image.url)
        return "No image"
    image_preview.short_description = 'Preview'
