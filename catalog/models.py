...
class Product(models.Model):
    ...
    description = models.TextField(
        help_text="Main product description (shared across variants)"
    )
    ...
    featured = models.BooleanField(
        default=False, help_text="Highlight on homepage"
    )
    sort_order = models.IntegerField(
        default=0, help_text="Manual ordering within category"
    )
...
class DigitalVariant(models.Model):
    ...
    requires_key = models.BooleanField(
        default=True, help_text="Whether this variant needs a game key"
    )
    ...
    sort_order = models.IntegerField(
        default=0, help_text="Display order within the product's variants"
    )
    sku = models.CharField(
        max_length=64,
        blank=True,
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.sku or not self.sku.strip():
            ...
            i = 1
            while (
                DigitalVariant.objects.filter(sku=sku)
                .exclude(pk=self.pk)
                .exists()
            ):
                i += 1
...
class Wishlist(models.Model):
    ...
    added_at = models.DateTimeField(
        auto_now_add=True, help_text="When product was added to wishlist"
    )
    ...
    if created:
        if instance.product_type == Product.CURRENCY:
            CurrencyProduct.objects.get_or_create(
                product=instance, defaults={"credit_amount": 0}
            )
