...
def currency_detail(request, product_slug=None):
    ...
    currency_products = (
        Product.objects.filter(product_type="currency", is_active=True)
    ...
    if product_slug:
        try:
            selected_product = Product.objects.get(
                slug=product_slug, product_type="currency", is_active=True
            )
            page_info = {
...
def edition_detail(request, product_slug, platform=None, edition=None):
    ...
    base_product = get_object_or_404(
        Product, slug=product_slug, product_type="base_game", is_active=True
    )

    # get all active variants for this product
    variants = base_product.digital_variants.filter(is_active=True).order_by(
        "sort_order", "platform", "edition"
    )
...
def search_results(request):
    ...
    if query:
        ...
        products = (
            ...
            .select_related("currency")
            .prefetch_related("digital_variants")
        )
