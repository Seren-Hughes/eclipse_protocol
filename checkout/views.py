...
def checkout(request):
    ...
    if request.user.is_authenticated:
        try:
            ...
            cart_items = list(
                db_cart.items.select_related(
                    "product", "variant", "product__currency"
                ).all()
            )
    ...
    saved_addresses = Address.objects.filter(
        user=request.user, address_type=Address.BILLING
    )
...
def process_payment(request):
    ...
    if not billing_address_data:
        messages.error(
            request, "Session expired. Please start checkout again."
        )
        return redirect("checkout:checkout")
    ...
    try:
            ...
            "items": [
                {
                    ...
                    "variant_id": (
                        str(item["variant"].id) if item["variant"] else None
                    ),
                    "variant_details": (
                        {
                            "platform": (
                                item["variant"].get_platform_display()
                                if item["variant"]
                                else None
                            ),
                            "edition": (
                                item["variant"].get_edition_display()
                                if item["variant"]
                                else None
                            ),
                        }
        ...
        order = Order.objects.create(
            ...
            stripe_pid=request.POST.get("client_secret", "").split("_secret")[
                0
            ],
...
def checkout_success(request, order_number):
    ...
    try:
        ...
        messages.error(
            request, "There was an error loading your order confirmation."
        )
        return redirect("home")
