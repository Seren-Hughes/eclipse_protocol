...
class Order(models.Model):
    ...
    )  # JSON snapshot of cart at purchase time
...
class OrderItem(models.Model):
    ...
    variant = models.ForeignKey(
        DigitalVariant, on_delete=models.PROTECT, null=True, blank=True
    )
...
class Payment(models.Model):
    ...
    status = models.CharField(
        max_length=30, choices=STATUS_CHOICES, default=REQUIRES_PAYMENT_METHOD
    )
...
class LicenseKey(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="license_keys"
    )
    order_item = models.ForeignKey(
        OrderItem, on_delete=models.CASCADE, related_name="license_keys"
    )
    product = models.ForeignKey(Product, on_delete=models.PROTECT)
    variant = models.ForeignKey(
        DigitalVariant, on_delete=models.PROTECT, null=True, blank=True
    ...
    platform = models.CharField(
        max_length=20, choices=PLATFORM_CHOICES, default=PC
    )
    ...
    status = models.CharField(
        max_length=20, choices=KEY_STATUS_CHOICES, default=KEY_ACTIVE
    )
    ...
    def __str__(self):
        product_name = (
            self.variant.product.name if self.variant else self.product.name
        )
    ...
    def record_download(self):
        ...
        self.save(
            update_fields=["download_count", "last_downloaded", "status"]
        )
