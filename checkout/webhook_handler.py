    ...
    def _send_confirmation_email(self, order):
        ...
        try:
            ...
            context = {
                ...
                "has_base_games": order.items.filter(
                    product__product_type="base_game"
                ).exists(),
                "has_currency": order.items.filter(
                    product__product_type="currency"
                ).exists(),
            }
    ...
    def handle_payment_intent_succeeded(self, event):
        ...
        for attempt in range(1, 6):
            try:
                ...
                if attempt == 5:
                    return HttpResponse(
                        content=f"Order not found for payment {pid}",
                        status=400,
                    )
                time.sleep(1)
