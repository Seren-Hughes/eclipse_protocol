...
class SupportModelTests(TestCase):
    ...
    def test_contact_message_meta_options(self):
        ...
        self.assertEqual(
            ContactMessage._meta.verbose_name_plural, "Contact Messages"
        )
...
class SupportViewTests(TestCase):
    ...
    def test_contact_form_submission_creates_message(self):
        ...
        self.assertEqual(
            contact_message.message, "This is a test message for support."
        )
        self.assertEqual(contact_message.status, "new")
    ...
    def test_contact_form_with_all_categories(self):
        ...
        for category in categories:
            ...
            response = self.client.post(
                reverse("support:contact"), data=form_data
            )
...
@override_settings(
    ...
    DEFAULT_FROM_EMAIL="noreply@testserver.com",
    ADMINS=[
        ("Admin One", "admin1@testserver.com"),
        ("Admin Two", "admin2@testserver.com"),
    ],
...
class SupportEmailTests(TestCase):
    ...
    def test_contact_submission_sends_admin_notification(self):
        ...
        for email in mail.outbox:
            if any(
                admin in email.to
                for admin in ["admin1@testserver.com", "admin2@testserver.com"]
            ):
                admin_email = email
                break
...
class SupportAdminTests(TestCase):
    ...
    def test_contact_message_admin_search(self):
        ...
        self.assertEqual(
            ContactMessageAdmin.search_fields, expected_search_fields
        )
...
class SupportIntegrationTests(TestCase):
    ...
    def test_complete_support_workflow(self):
        ...
        user_email = next(
            (
                email
                for email in mail.outbox
                if "alice@example.com" in email.to
            ),
            None,
        )
        admin_email = next(
            (
                email
                for email in mail.outbox
                if "support@eclipseprotocol.com" in email.to
            ),
            None,
        )
    ...
    def test_multiple_contact_submissions_same_user(self):
        ...
        response = self.client.post(
            reverse("support:contact"), data=form_data1
        )
        self.assertEqual(response.status_code, 302)
        ...
        response = self.client.post(
            reverse("support:contact"), data=form_data2
        )
        self.assertEqual(response.status_code, 302)

        # Verify both messages were created
        messages = ContactMessage.objects.filter(email=user_email).order_by(
            "-created_at"
        )
        self.assertEqual(messages.count(), 2)

        self.assertEqual(
            messages[0].subject, "Second Issue"
        )  # Most recent first
